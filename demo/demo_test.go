...
`, demoPort, apiKey, dbPath)
	...
	s4 := createSession(t, serverURL, adminKey, "Create a table crash_test with a single column 'survived', insert one row with value 'yes', and record a text_block memory event confirming the committed result.")
	sendMsg(t, serverURL, adminKey, s4, "Commit the crash test task and its memory record now.")
	fmt.Printf("   Session %s created — waiting for one committed agent artifact...\n", s4[:20]+"...")
	if !waitForStatus(t, serverURL, adminKey, s4, []string{"completed", "idle", "failed"}, 30*time.Second) {
		t.Fatal("crash recovery setup did not reach a terminal status")
	}

	// Establish committed progress before the crash. User input alone is not
	// sufficient evidence: the snapshot must contain an agent memory row,
	// iteration commit, or committed audit row.
	preRecovery := readRecoverySnapshot(t, dbPath, s4)
	if err := recoverydiag.VerifyDurableAgentProgress(preRecovery, preRecovery); err != nil {
		t.Fatalf("crash recovery setup: %v (%s)", err, preRecovery.Diagnostic())
	}
	fmt.Printf("   Pre-crash committed evidence: %s\n", preRecovery.Diagnostic())

	// Wake a second iteration and observe the durable planning claim before
	// killing the process. This is an in-flight crash, not a post-completion kill.
	sendMsg(t, serverURL, adminKey, s4, "Begin a second transaction that creates crash_test_inflight, but do not reuse the previous result.")
	inFlightStatus := waitForInFlightStatus(t, serverURL, adminKey, s4, 20*time.Second)
	if inFlightStatus == "" {
		t.Fatal("session never entered an in-flight status; refusing to claim a crash-recovery proof")
	}
	...
	crashRecovery := readRecoverySnapshot(t, dbPath, s4)
	if err := recoverydiag.VerifyDurableAgentProgress(preRecovery, crashRecovery); err != nil {
		t.Fatalf("committed evidence lost at crash boundary: %v", err)
	}
	fmt.Printf("   Recovery diagnostics after abnormal exit: %s\n", crashRecovery.Diagnostic())
	...
	postRecovery := readRecoverySnapshot(t, dbPath, s4)
	if err := recoverydiag.VerifyDurableAgentProgress(preRecovery, postRecovery); err != nil {
		t.Fatalf("crash recovery proof failed: %v (pre: %s; post: %s)",
			err, preRecovery.Diagnostic(), postRecovery.Diagnostic())
	}
	fmt.Printf("   ✓ Committed agent artifact survived restart: %s\n", postRecovery.Diagnostic())
	fmt.Println("   ℹ In-flight transaction is not claimed as durable; staged/executed residue is reported above")
	...
	fmt.Println("║  Committed agent progress survives restart                  ║")
...
func readRecoverySnapshot(t *testing.T, dbPath, sessionID string) recoverydiag.Snapshot {
	t.Helper()
...
func waitForInFlightStatus(t *testing.T, url, key, sid string, timeout time.Duration) string {
	t.Helper()
	deadline := time.Now().Add(timeout)
	for time.Now().Before(deadline) {
		session := getSession(t, url, key, sid)
