...
import (
	...
	"github.com/wojons/consensus/internal/recoverydiag"
)
...
`, port, apiKey, dbPath)
	...
	// === PHASE 1: Commit agent progress, then crash a second in-flight iteration ===
	...
	sendMessage(t, baseURL, adminKey, sessionID,
		"Create a table called recovery_test (id INTEGER PRIMARY KEY AUTOINCREMENT, note TEXT). "+
			"Write a text_block to memory_events confirming the table was created, then commit.")

	firstResult := pollSessionComplete(t, baseURL, adminKey, sessionID, 120*time.Second)
	if firstResult.Status != "idle" && firstResult.Status != "completed" {
		t.Fatalf("setup iteration did not commit: status=%s events=%d", firstResult.Status, firstResult.EventCount)
	}
	preCrash := readHarnessRecoverySnapshot(t, dbPath, sessionID)
	if err := recoverydiag.VerifyDurableAgentProgress(preCrash, preCrash); err != nil {
		t.Fatalf("setup produced no committed agent progress: %v (%s)", err, preCrash.Diagnostic())
	}
	t.Logf("  committed pre-crash evidence: %s", preCrash.Diagnostic())

	sendMessage(t, baseURL, adminKey, sessionID,
		"Begin a second transaction that creates recovery_test_inflight and records its result.")
	inFlightStatus := waitForHarnessInFlightStatus(t, baseURL, adminKey, sessionID, 20*time.Second)
	if inFlightStatus == "" {
		t.Fatal("session never entered an in-flight status; refusing an input-only crash proof")
	}
	t.Logf("  observed in-flight status=%s", inFlightStatus)
	...
	if err := recoverydiag.VerifyDurableAgentProgress(preCrash, crashBoundary); err != nil {
		t.Fatalf("committed progress lost at crash boundary: %v", err)
	}
	t.Logf("  recovery diagnostics after abnormal exit: %s", crashBoundary.Diagnostic())
	...
	postCrash := readHarnessRecoverySnapshot(t, dbPath, sessionID)
	if err := recoverydiag.VerifyDurableAgentProgress(preCrash, postCrash); err != nil {
		t.Fatalf("durable agent progress did not survive restart: %v (pre: %s; post: %s)",
			err, preCrash.Diagnostic(), postCrash.Diagnostic())
	}
	t.Logf("  post-restart evidence: %s", postCrash.Diagnostic())

	// The open transaction is expected to roll back. The session may remain in
	// planning/tool_exec until heartbeat recovery or stale-session reaping; its
	// staging residue is diagnostic evidence, not a successful commit.
	recoveredSession := querySession(t, baseURL, adminKey, sessionID)
	if recoveredSession.Status == "planning" || recoveredSession.Status == "thinking" || recoveredSession.Status == "tool_exec" {
		recoveredSession = pollSessionComplete(t, baseURL, adminKey, sessionID, 120*time.Second)
	}
	if recoveredSession.Status != "idle" && recoveredSession.Status != "completed" && recoveredSession.Status != "failed" {
		t.Errorf("session stuck after crash recovery: status=%s", recoveredSession.Status)
		return
	}

	t.Logf("✓ Crash recovery test PASSED — committed agent artifacts survived; session %s",
...
func readHarnessRecoverySnapshot(t *testing.T, dbPath, sessionID string) recoverydiag.Snapshot {
	t.Helper()
	ctx := context.Background()
	database, err := driver.Open(ctx, db.Config{URL: "sqlite://" + dbPath, MaxOpenConns: 1})
	if err != nil {
...
func waitForHarnessInFlightStatus(t *testing.T, baseURL, adminKey, sessionID string, timeout time.Duration) string {
	t.Helper()
	deadline := time.Now().Add(timeout)
	for time.Now().Before(deadline) {
		status := querySession(t, baseURL, adminKey, sessionID).Status
		switch status {
		case "planning", "tool_exec", "executing":
			return status
		case "failed", "paused":
			return ""
		}
