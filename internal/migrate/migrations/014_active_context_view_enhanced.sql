...
--
-- The VIEW is session-isolated via current_setting('consensus.session_id').
...
page_direct_ids AS (
    ...
    WHERE mp.session_id = current_setting('consensus.session_id')::UUID
),
...
page_linked_ids AS (
    ...
    WHERE mp1.session_id = current_setting('consensus.session_id')::UUID
),
