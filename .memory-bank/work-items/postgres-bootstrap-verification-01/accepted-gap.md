...
The Postgres-specific bootstrap verification test (`CONSCIENCE_TEST_POSTGRES_URL=<url> go test ./internal/bootstrap -run Postgres -v -count=1`) is not runnable in the current CI environment because no Postgres instance is available.
...
- Adding a gated Postgres test would require either: (a) a running Postgres instance in CI, or (b) a test that skips when `CONSCIENCE_TEST_POSTGRES_URL` is unset. The latter is the correct approach when Postgres CI is available.

## Action Required to Close This Gap

When a Postgres CI environment becomes available:
1. Add `CONSCIENCE_TEST_POSTGRES_URL` to CI environment variables
