...
| Init bootstrap | `go run ./cmd/consensus init --db-url sqlite://dev.db --format json` | gap | Printed help text only; no admin key created |
...
| Fresh `init` bootstrap probe | `go run ./cmd/consensus init --db-url sqlite://_tmp/runtime-dev-bootstrap-auth.db` plus authenticated `/api/v1/sessions` probe | pass | `init_key=cs_ak_cc[REDACTED]`, `admin_key_rows=1`, `authenticated_sessions_status=200`. Raw secret was captured only in `_tmp/runtime-dev-bootstrap-auth.init.out` for local probe use; report redacts it. |
| Fresh `serve` bootstrap probe | `CONSENSUS_DB_URL=sqlite://_tmp/runtime-serve-bootstrap-auth.db go run ./cmd/consensus serve --port 18091` plus authenticated `/api/v1/sessions` probe | pass | `health=200`, `serve_key=cs_ak_43[REDACTED]`, `admin_key_rows=1`, `authenticated_sessions_status=200`. Raw secret stayed in local `_tmp/runtime-serve-bootstrap-auth.server.log`; report redacts it. |
| Trace repair | `axiom:trace` headers in changed implementation/test files | pass | `internal/bootstrap/admin_key.go`, `internal/bootstrap/admin_key_test.go`, `internal/cli/init.go`, and `cmd/consensus/main.go` reference `runtime-dev-bootstrap-auth-01`. |

## Gap

Fresh local runtime is now self-bootstrapping for authenticated testing through both `consensus init` and first `consensus serve`. The first admin key is stored hashed and printed only when created.
