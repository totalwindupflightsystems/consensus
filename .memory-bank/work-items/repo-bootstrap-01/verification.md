...
| AC-4 | North-star traceability (SPEC-000) | **PASS** | Trace markers on all boundary files: `cmd/conscience/main.go`, `internal/db/db.go`, `internal/db/dispatch.go` |
...
ok  	github.com/wojons/conscientiousness/internal/config	0.249s
ok  	github.com/wojons/conscientiousness/internal/db	0.388s
...
$ ls -la bin/conscience
-rwxr-xr-x  1 user  staff  3579266 May  3 21:48 bin/conscience
```

## Artifacts Produced

- `go.mod` / `go.sum` — Module definition with gopkg.in/yaml.v3
- `cmd/conscience/main.go` — Entry point with config + db + signal handling
...
- `conscience.yaml` — Default configuration
- `bin/conscience` — Compiled binary
