...
## Current Focus
The Consensus platform is fully built end-to-end. 27 Go packages, all build and test clean. All red-team gaps (RT-001 through RT-007) are now remediated. The platform has real LLM providers (OpenAI/Anthropic HTTP clients), complete MCP subsystem with trace markers, updated spec inventory, and baseline operational runbooks.

## Recent Changes
- **2026-06-02**: make-consensus-fully-operational-end-to — SPEC-021 file inventory updated, MCP trace markers added, troubleshooting runbook created, plan cursor advanced
...
- **LLM providers are real HTTP clients** — `llm.NewClient(cfg)` factory selects provider (openai/anthropic/mock/openrouter) via CONSENSUS_LLM_PROVIDER env var. OpenAI client supports response_format (json_object, json_schema strict). Anthropic client supports prompt caching via cache_control ephemeral. No SDK dependencies.
...
## Recent Completion
- **2026-06-02**: make-consensus-fully-operational-end-to — all phases complete.
...
### Remediated
- RT-001: Real LLM provider integration — OpenAI and Anthropic HTTP clients implemented with Structured Outputs and prompt caching. Production `consensus serve` uses `llm.NewClient()` with configurable provider.
