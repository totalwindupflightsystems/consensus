...
version: "1.0"
synopsis: |
  DeeDee is the QA sibling of AutoTune. Same Morty template engine, same Rick supervision, same
  ...
  Source spec: specs/99-DeeDee-QA-Harness.md.
when-to-use: |
...
# deedee-qa-harness-axiom — Scenario-Based QA via Morty Templates
...
DeeDee relates to AutoTune and the local dev test harness.

**Source spec**: `specs/99-DeeDee-QA-Harness.md`
**Design doc**: `.memory-bank/projects/axiom/deedee-autotune.md`
**Work item**: `deedee-qa-harness-01`
...
| `rick-and-morty-axiom` | Rick supervises DeeDee runs the same way it supervises AutoTune sweeps |
| `axiom-local-dev-test-harness` | Consumer — DeeDee uses it as the `pytest` runner type |
| `performance-benchmark-axiom` | Peer — AutoTune uses it for benchmarks; DeeDee may use it for perf scenarios |
...
2. **Per-stage model declaration is a Morty-level feature** — not DeeDee-specific; benefits AutoTune too
...
5. **DeeDee consumes `axiom-local-dev-test-harness`** — they're peers at different layers

axiom:trace work_item=deedee-qa-harness-01 spec=specs/99-DeeDee-QA-Harness.md
