...
import { loadPluginConfig, pluginWarn, pluginError, pluginInfo } from "./config-utils.ts";
import { LOCK_TTL_MS } from "../shared/stash-constants.ts";

// Re-export advisory lock TTL constant for test access (REQ-STASH-NEW-008)
// axiom:trace work_item=SWDE-55 spec=specs/106-Context-Stash.md#REQ-STASH-NEW-008
export { LOCK_TTL_MS } from "../shared/stash-constants.ts";
 ...
 */
// Exported from .opencode/shared/stash-constants.ts for tests.
// Kept here as module-local for plugin use (plugin loader crashes on non-function exports).
const STASH_ID_REGEX = /^[a-z0-9][a-z0-9-]{0,63}$/;
...
export function buildSuspendedMarkdown(
  ...
  summary: string | undefined,  // axiom:trace work_item=plugin-bug-sweep-01 spec=specs/106-Context-Stash.md#REQ-STASH-001 plan=phase-1/step-2
  detail?: string,
  resumeHint?: string
): string {
  summary = summary ?? "";  // null-safe: summary may be undefined when called outside OpenCode schema validation
...
// Exported per spec §8.3 (GAP-5: plugins expose their DEFAULT_CONFIG).
// axiom:trace work_item=plugin-config-management-01 spec=specs/112-Plugin-Config-Management.md#8.3 plan=phase-final/step-final-03
...
function logLifecycleEvent(event: { type: string; stash_id: string; state?: string; agent?: string; [key: string]: unknown }): void {
  // axiom:trace work_item=SWDE-55 spec=specs/106-Context-Stash.md#REQ-STASH-093
  // Env-gated: writes to stderr only when AXIOM_CONTEXT_STASH_DEBUG=1.
  // Without this, lifecycle events flooded the OpenCode TUI on every stash op.
  if (process.env.AXIOM_CONTEXT_STASH_DEBUG !== "1") return;
  process.stderr.write(JSON.stringify({ source: "context-stash", ts: new Date().toISOString(), ...event }) + "\n");
}
...
export class LocalFileBackend implements StashStorageBackend {
  ...
  async append(stashId: string, entry: StashEntry): Promise<{ warning?: string }> {
    ...
    if (!existsSync(filePath)) {
      throw new Error(`No active stash '${stashId}'. Use stash_enter to activate a stash first.`);
    }
...
// For test access, import from .opencode/shared/stash-constants.ts instead.
...
export class FallbackBackend implements StashStorageBackend {
  ...
  async write(stashId: string, content: StashContent): Promise<void> {
    try { return await this.primary.write(stashId, content); }
      ...
      pluginWarn("context-stash", `Backend write error — falling back. Error: ${err instanceof Error ? err.message.slice(0, 80) : String(err).slice(0, 80)}`);
  ...
  async moveToClose(stashId: string, closedContent: StashContent): Promise<{ warning?: string }> {
    try { return await this.primary.moveToClose(stashId, closedContent); }
      ...
      pluginWarn("context-stash", `Backend moveToClose error — falling back. Error: ${err instanceof Error ? err.message.slice(0, 80) : String(err).slice(0, 80)}`);
  ...
  async append(stashId: string, entry: StashEntry): Promise<{ warning?: string }> {
    // axiom:trace work_item=SWDE-55 spec=specs/106-Context-Stash.md#REQ-STASH-NEW-013 plan=phase-4/fix-swde55-append-interface
    // axiom:trace work_item=SWDE-59 spec=specs/106-Context-Stash.md#REQ-STASH-NEW-017 plan=phase-6-post-verify-run3/add-fallbackbackend-append-warning
    // If primary implements append, delegate to it. On failure, fall back to local
    // (consistent with all other FallbackBackend methods). If primary omits append,
    // fall back to local silently — append is optional per the interface contract.
    try {
      if (this.primary.append) {
        return await this.primary.append(stashId, entry);
      }
    } catch (err) {
      pluginWarn("context-stash", `Backend append error — falling back. Error: ${err instanceof Error ? err.message.slice(0, 80) : String(err).slice(0, 80)}`);
...
function createDefaultBackend(
  ...
  if (backendType === "s3") {
    ...
    if (bucket && clientFactory) {
      ...
      if (s3Client) {
        pluginInfo("context-stash", `Using S3Backend (bucket: ${bucket}, prefix: ${prefix})`);
    ...
    if (dsn && clientFactory) {
      ...
      if (pgClient) {
        pluginInfo("context-stash", `Using PostgresBackend`);
        return new FallbackBackend(
    ...
    pluginError("context-stash", `Unknown STASH_BACKEND='${backendType}' — falling back to local file storage.`);
  }
...
export const ContextStashPlugin = async ({
  ...
  if (existsSync(yamlConfigPath) && !existsSync(jsonConfigPath)) {
    try {
      ...
      if (parsed.stash && Object.keys(parsed.stash as object).length > 0) {
        ...
        pluginWarn("context-stash",
          "MIGRATION REQUIRED: Your stash config in .axiom/axiom.config.yaml " +
  ...
  pluginInfo("context-stash", `Initialized — storage: ${storageRoot}`);
  ...
  return {
    tool: {
      ...
      "stash_push": tool({
        ...
        async execute({ name, summary, tags, detail, resume_hint }, context) {
          try {
            // Guard: required params may be undefined when called outside OpenCode schema validation
            // axiom:trace work_item=plugin-bug-sweep-01 spec=specs/106-Context-Stash.md#REQ-STASH-001 plan=phase-1/step-2
            if (!name) return JSON.stringify({ error: "name is required" });
            if (!summary) return JSON.stringify({ error: "summary is required — provide a 1-3 sentence description of this context" });
            ...
            logLifecycleEvent({ type: "stash_created", stash_id: stashId, state: "suspended", agent });
            return JSON.stringify({
      ...
      "stash_drop": tool({
        ...
        async execute({ id }) {
          try {
            ...
            logLifecycleEvent({ type: "stash_dropped", stash_id: id });
            return JSON.stringify({
      ...
      "stash_close": tool({
        ...
        async execute({ id, outcome }, context) {
          try {
            ...
            logLifecycleEvent({ type: "stash_closed", stash_id: id, state: "closed", agent });
            if (warning) {
              return JSON.stringify({
        ...
        async execute({ id, name, summary, log_level, background }, context) {
          try {
            ...
            if (id) {
              ...
              logLifecycleEvent({ type: "stash_activated", stash_id: stashId, state: "active", agent });
              return JSON.stringify({
                stash_id: stashId, name: stashName, state: "active",
      ...
      "stash_exit": tool({
        ...
        async execute({ id, resume_hint, outcome_summary }, context) {
          try {
            ...
            logLifecycleEvent({ type: "stash_suspended", stash_id: id, state: "suspended", agent });
            return JSON.stringify({
              stash_id: id, state: "suspended",
              entries_count: entriesCount,
              file: `suspended/${id}.md`,
              resume_hint: resume_hint ?? fm.resume_hint,
              message: `Stash '${id}' suspended with ${entriesCount} log entries. Resume with: stash_enter --id ${id}`,
            });
          } catch (err) {
            return JSON.stringify({ error: err instanceof Error ? err.message : String(err) });
          }
      ...
      "stash_append": tool({
        description:
          ...
          "The stash must be active (use stash_enter first). " +
        ...
        async execute({ id, keep_types, keep_last = 10 }) {
          try {
            ...
            const activeFm: StashFrontmatter & { session_id: string; entered_at: string } = {
              ...
              name: fm.name ?? id,
              state: "active" as any,
              created_by: fm.created_by ?? "stash_compact",
              created_at: fm.created_at ?? new Date().toISOString(),
      ...
      "stash_lock": tool({
        description:
         ...
         args: {
          ...
          ttl_seconds: tool.schema.number().optional().describe(`Lock TTL in seconds (default: ${LOCK_TTL_MS / 1000})`),
        },
        async execute({ id, ttl_seconds = LOCK_TTL_MS / 1000 }, context) {
          try {
            // axiom:trace work_item=SWDE-55 spec=specs/106-Context-Stash.md#REQ-STASH-NEW-008 plan=phase-4-backlog/bl-r9-5
            if (ttl_seconds < 30) {
              return JSON.stringify({ error: `ttl_seconds must be >= 30 (got ${ttl_seconds}). Use LOCK_TTL_MS/1000 (${LOCK_TTL_MS/1000}s) for the spec-compliant default.` });
            }
            validateStashId(id);
      ...
      "stash_archive": tool({
        ...
        async execute({ id }) {
          try {
            ...
            logLifecycleEvent({ type: "stash_archived", stash_id: id });
            return JSON.stringify({
    ...
    "tool.execute.after": async (input: { tool: string; args?: Record<string, unknown> }, output: unknown) => {
      ...
      if (String(input.tool).startsWith("stash_")) return;
    ...
    "session.created": async (_event: unknown) => {
      ...
      try {
        ...
        pluginInfo("context-stash", `Auto-entered stash '${autoEnterId}' (log_level: ${logLevel})`);
