...
import { pluginWarn, pluginError, pluginInfo } from "./config-utils.ts";
...
export async function loadDuckdbIfAvailable(projectRoot: string): Promise<DuckdbAdapter | null> {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    pluginWarn("tree-memory", `WARN: DuckDB load failed (${msg}), falling back to filesystem scanner`);
    return null;
  }
}
...
export const TreeMemoryPlugin = async ({
  ...
  // ─── Helper: getDefaultBranch ────────────────────────────────────────────────
  // axiom:trace work_item=plugin-bug-sweep-01 spec=specs/113-Tree-Memory.md#5.3 plan=phase-1/task-1-3/step-backlog-003
  /**
   * Detect the repo's default branch at runtime so tree.diff works in repos that
   * use "trunk", "master", or any other default branch name instead of "main".
   *
   * Lookup order:
   *   1. git symbolic-ref --short refs/remotes/origin/HEAD  (most reliable — reflects remote)
   *   2. git config init.defaultBranch                      (local config fallback)
   *   3. "main"                                             (final hardcoded fallback + warn)
   */
  function getDefaultBranch(repoDir: string): string {
    try {
      // Try remote HEAD first (most reliable)
      const result = execSync("git symbolic-ref --short refs/remotes/origin/HEAD", {
        cwd: repoDir, encoding: "utf8", timeout: 3000, stdio: ["pipe", "pipe", "pipe"],
      }).trim();
      // Returns e.g. "origin/main" → strip "origin/" prefix
      if (result.includes("/")) return result.split("/").slice(1).join("/");
      return result;
    } catch {
      try {
        // Fall back to git config
        const result = execSync("git config init.defaultBranch", {
          cwd: repoDir, encoding: "utf8", timeout: 3000, stdio: ["pipe", "pipe", "pipe"],
        }).trim();
    ...
    // Final fallback
    console.warn("[TreeMemory] Could not detect default branch, falling back to 'main'");
    return "main";
  }

  // ─── Tool: tree.diff ─────────────────────────────────────────────────────────
  // axiom:trace work_item=tree-memory-01 spec=specs/113-Tree-Memory.md#5.3 plan=phase-1/task-1-2/step-1-2-3
  const treeDiff = tool({
    // axiom:trace work_item=plugin-bug-sweep-01 spec=specs/113-Tree-Memory.md#5.3 plan=phase-1/task-1-3/step-backlog-003
    description:
      ...
      "Pass only branch_a to diff it against the repo's default branch (detected dynamically).",
    args: {
      branch_a: tool.schema.string().describe("Branch to compare (required)"),
      branch_b: tool.schema.string().optional().describe("Branch to compare against (default: repo default branch, e.g. main/trunk/master)"),
    },
    async execute({ branch_a, branch_b }) {
      try {
        if (!branch_a) {
          return JSON.stringify({ error: "branch_a is required. Pass branch_a and optionally branch_b (defaults to repo default branch)." });
        }
        const g = ensureGit();
        const b = branch_b ?? getDefaultBranch(projectRoot);
  ...
  return {
    tool: {
      ...
      "tree_commit": treeCommit,
      "tree_promote": treePromote,
      "tree_merge": treeMerge,
      "tree_state": treeState,
      "tree_query": treeQuery,
      "tree_peers": treePeers,
      "tree_log": treeLog,
      "tree_diff": treeDiff,
      "tree_spawn": treeSpawn,
