...
export const DEFAULT_CONFIG: ConductorConfig = { // axiom:trace work_item=plugin-config-management-01 spec=specs/112-Plugin-Config-Management.md#8 plan=phase-4/task-4-2/step-4-2-0
  ...
  auth: {
    ...
    allow_spawn_secret_fallback: true,  // axiom:trace work_item=plugin-bug-sweep-01 spec=specs/107-Conductor.md#REQ-COND-005b plan=phase-3/step-3 — default true for local dev; production overrides to false via config file
...
export const ConductorPlugin = async ({
  ...
  try {
    ...
    console.warn(`[conductor] loadPluginConfig failed (${msg}), using DEFAULT_CONFIG`);
  ...
  console.debug("[conductor] Effective config loaded:", JSON.stringify({
    limits: config.limits,
    polling: config.polling,
    lifecycle: config.lifecycle,
    context_banner: config.context_banner,
    // NOTE: auth fields omitted from debug log (may contain sensitive path info)
  }));
  ...
  // axiom:trace work_item=plugin-bug-sweep-01 spec=specs/107-Conductor.md#REQ-COND-005b plan=phase-3/step-3
  // BUG-9 fix: degrade gracefully instead of throwing at init time.
  // When SPIRE is unavailable and fallback is disabled, log a WARN and continue.
  // conductor.spawn will fail at call-time (not here) so all tools remain registered.
  let spireUnavailableNoFallback = false;
  if (!spireAvailable && !fallbackAllowed) {
    spireUnavailableNoFallback = true;
    console.warn(
      "[Conductor] WARN: SPIRE unavailable and spawn_secret_fallback disabled. " +
      "conductor.spawn will fail until SPIRE is available or allow_spawn_secret_fallback is enabled. " +
      "Set allow_spawn_secret_fallback=true for local dev or ensure SPIRE agent socket is available."
    );
    // Don't throw — continue initializing and registering all tools
  }

  if (!spireAvailable && fallbackAllowed) {
    console.warn(
      "[Conductor] WARNING: Using spawn secret fallback (SPIRE unavailable). " +
  ...
  async function spawnSession(
    ...
    try {
      ...
      if (resp.ok) {
        ...
        const sessionId = (body.id ?? body.session_id ?? body.sessionID) as
          | string
          | null;
      ...
      /* HTTP unavailable — fall through to generated ID */
    }
      ...
      try {
        ...
        if (resp.ok) {
          ...
          if (Array.isArray(body)) {
            messages = body as Record<string, unknown>[];
          } else {
            const b = body as Record<string, unknown>;
            const arr = b.messages ?? b.data ?? b.content;
            if (Array.isArray(arr)) messages = arr as Record<string, unknown>[];
          }
          const assistantMessages = messages.filter(
            (m) => String(m.role ?? m.type ?? "").toLowerCase() === "assistant"
          );
          if (assistantMessages.length > 0) {
            const last = assistantMessages[assistantMessages.length - 1];
            return String(last.content ?? last.text ?? last.message ?? "");
          }
        }
      } catch {
        /* try next */
  ...
  function emitLifecycleEvent(
    ...
    // Do NOT log at console.log level — use console.debug so it's not surfaced by default
    console.debug(`[Conductor] ${JSON.stringify(event)}`);
  }
  ...
  const conductorSpawnTool = tool({
    ...
    async execute(args, context) {
      ...
      try {
        // ── BUG-9 call-time auth gate (REQ-COND-005b) ──────────────────────
        // axiom:trace work_item=plugin-bug-sweep-01 spec=specs/107-Conductor.md#REQ-COND-005b plan=phase-3/step-3
        if (spireUnavailableNoFallback) {
          return JSON.stringify({
            error: "conductor.spawn unavailable: SPIRE is not running and allow_spawn_secret_fallback=false. " +
              "Set allow_spawn_secret_fallback=true for local dev or ensure the SPIRE agent socket is available.",
            code: 503,
          });
        }
        ...
        if (!spireAvailable && fallbackAllowed) {
          console.error(
            `[Conductor] CRITICAL: ${JSON.stringify({
        ...
        }, "conductor.spawn:insert_agent");
        if (!insertOk) {
          console.warn(`[Conductor] Failed to record agent ${agentId} in DB after retries — agent is running but untracked`);
        }
  ...
  const conductorCancelTool = tool({
    ...
    async execute(args) {
      ...
      try {
        ...
        }, "conductor.cancel:update_status");
        ...
        for (const agent of runningAgents) {
          ...
          if (agent.timeout_at) {
            ...
            if (now > timeoutAt) {
              ...
              }, "conductor.wait:update_completed");
    ...
    async execute(args, context) {
      try {
        ...
        await Promise.all(runningAgents.map(async (agent) => {
          try {
            const resp = await fetch(`${baseUrl}/session/${agent.session_id}/message`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ role: "user", content: args.message }),
              signal: AbortSignal.timeout(3000),
            });
            if (resp.ok) {
              sent++;
  ...
  return {
    tool: {
      "conductor.spawn": conductorSpawnTool,
      "conductor.done": conductorDoneTool,
      "conductor.status": conductorStatusTool,
      "conductor.result": conductorResultTool,
      "conductor.cancel": conductorCancelTool,
      "conductor.wait": conductorWaitTool,
      "conductor.collect": conductorCollectTool,
      "conductor.broadcast": conductorBroadcastTool,
      "conductor.delegate": conductorDelegateTool,
      "conductor.relay": conductorRelayTool,
      "conductor.focus": conductorFocusTool,
      "conductor.unfocus": conductorUnfocusTool,
      "conductor.pin": conductorPinTool,
    },
              ...
              await Promise.all(
                runningAgents.map(async (agent) => {
                  try {
                    ...
                    }, "conductor.cleanup:update_cancelled");
