...
planning ──► failed          (planning error; waits for explicit user/operator recovery)
failed ──► idle              (explicit resume after correcting or retrying the failure)
failed ──► thinking          (a new user message resumes the session immediately)
